...
export const NavBarVisuallyImpaired = ({ locale, display, footer }) => {
  ...
  return (
    <NavVisually
      border={!visuallyImpairedModeWhiteTheme ? "white" : "black"}
      color={
        !visuallyImpairedModeWhiteTheme ? "white" : footer ? "white" : "black"
      }
      display={display}
    >
      <Choose>
        ...
        <Handle>
          <Switch>
            <InputCheckbox
              checked={images}
              onChange={() => null}
              onClick={() => dispatch(ClickOnOffImages())}
              type="checkbox"
            />
            <SliderRound />
          </Switch>
      ...
      <Choose>
        ...
        <Handle>
          <ColorStrong
            background="white"
            color="black"
            onClick={() => dispatch(ClickOnOffWhiteTheme(true))}
          >
            A
          </ColorStrong>
          <ColorStrong
            background="black"
            color="white"
            onClick={() => dispatch(ClickOnOffWhiteTheme(false))}
          >
            A
          </ColorStrong>
        </Handle>
...
export const NavBar = ({ navButtons, register, logIn }) => {
  ...
  return (
    <Nav>
              ...
              button.children.length > 0 ? (
                <li key={index}>
                  <a href="#" aria-haspopup="true">
                    {button.title} <i className="fa fa-caret-down" />
                  </a>
                    ...
                    {button.children.map((el, i) =>
                      el.path.charAt(0) === "#" ? (
                        <Link key={i} scroll={false} href="/">
                          <li
                            onClick={() => dispatch(ScrollToElement(el.path))}
                          >
                            <a>{el.title}</a>
                          </li>
                        </Link>
      ...
      <SignInMain top="45px">
        <Link href="/register">
          <RegisterLink color={color}>{register}</RegisterLink>
        </Link>
        <Link href="/logIn">
          <a>
      ...
      <ChangeLanguageAndSearchContainer>
        ...
        <SignIn>
          <AnimationSearchBarStyled
            ...
            inputFunc={(e) =>
              dispatch(OnchangeInputSearchNews(e.target.value, locale))
            }
