...
export const hexToRgbA = (hex) => {
  ...
  if (/^#([A-Fa-f0-9]{3}){1,2}$/.test(hex)) {
    ...
    c = "0x" + c.join("");
    return (
      "rgba(" + [(c >> 16) & 255, (c >> 8) & 255, c & 255].join(",") + ",0.05)"
    );
  }
...
export const cutUri = (uri) => {
  if (uri) {
    if (uri.substring(0, 4) === "/en/" || uri.substring(0, 4) === "/ru/") {
...
export const ParcUri = (uri) => {
  if (
    cutUri(uri) === "/financial-statements/" ||
...
export const registerOnEventHook = async (eventName, time, fName, lName) => {
  ...
  const eventDate = format(time, "dd/MM/yyyy");
  const data = {
    eventName,
    eventDate,
    eventTime,
...
export const sendAppeal = async (fName, lName, reason, email, text, file) => {
  const data = {
    fName,
    lName,
    email,
...
export const registerOnServiceHook = async (
  serviceName,
  fName,
  lName,
  phone
